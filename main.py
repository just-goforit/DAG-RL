...
def show_model_param_num(net: CustomMaskablePPO):
    ...
    print(
        "features_extractor: %.2fK"
        % (sum(p.numel() for p in net.policy.features_extractor.parameters()) / 1e3)
    )
    print(
        "mlp_extractor_net: %.2fK"
        % (sum(p.numel() for p in net.policy.mlp_extractor.parameters()) / 1e3)
    )
    if net.policy.action_net is not None:
        print(
            "action_net: %.2fK"
            % (sum(p.numel() for p in net.policy.action_net.parameters()) / 1e3)
        )
    if net.policy.value_net is not None:
        print(
            "value_net: %.2fK"
            % (sum(p.numel() for p in net.policy.value_net.parameters()) / 1e3)
        )
...
if __name__ == "__main__":
    ...
    env_config = {
        ...
        "obs_mode": (
            "vec" if args.model == "MLP" and args.node_wise == False else "box"
        ),  # mlp mode better to use vec obs_mode
    ...
    policy_kwargs = dict(
        ...
        addi_features=(
            0 if args.obs_field == "loc" or args.model == "MLP" else ADD_FEATURE
        ),
        ...
        net_arch=dict(
            pi=args.pi_net_arch, vf=args.vf_net_arch
        ),  # default is dict(pi=[64, 64], vf=[64, 64]) need
    )
    ...
    if args.clip_range_vf != None:
        clip_range_vf = (
            Linear_schedule(args.clip_range_vf[0], args.clip_range_vf[1])
            if args.clip_range_vf[0] != args.clip_range_vf[1]
            else args.clip_range_vf[0]
        )
    ...
    if not args.eval_mode and not args.human:
        ...
        model = CustomMaskablePPO(
            ...
            policy=(
                ...
                if args.model != "NONE"
                else MaskableActorCriticPolicy
            ),
            learning_rate=(
                Linear_schedule(args.lr[0], args.lr[1])
                if args.lr[0] != args.lr[1]
                else args.lr[0]
            ),
            ...
            device=args.device,
            policy_kwargs=(
                policy_kwargs if args.model != "NONE" else simple_policy_kwargs
            ),
            verbose=0,
        )
        if args.debug:
        ...
        if args.pretr != "":
            ...
            mname = (
                args.save_path + "/" + args.pretr.rsplit("_", 2)[0] + "/" + args.pretr
            )
            model = model.load(
                mname,
                venv,
        ...
        checkpoint_on_event = CheckpointCallback(
            save_freq=1, save_path=save_dir, name_prefix=model_name
        )
        event_callback = customEveryNTimesteps(
            n_steps=save_freq, callback=checkpoint_on_event
        )

        # 创建 tqdm 进度条回调
        tqdm_callback = TqdmProgressCallback(
            total_timesteps=int(args.total_timesteps), update_freq=args.n_steps
        )
        ...
        print("=" * 80)
        print(
            f"Total Rollout Time:  {model.total_rollout_time:.2f} seconds ({model.total_rollout_time/3600:.2f} hours)"
        )
        print(
            f"Total Train Time:    {model.total_train_time:.2f} seconds ({model.total_train_time/3600:.2f} hours)"
        )
        ...
        print(
            f"Avg Rollout Time:    {model.total_rollout_time/model.rollout_count:.2f} seconds"
        )
        print(
            f"Avg Train Time:      {model.total_train_time/model.train_count:.2f} seconds"
        )
    ...
    if args.eval_mode:
        ...
        mname = (
            save_dir + "/" + model_name + f"_{args.total_timesteps}_steps"
            if not args.eval_mode
            else args.save_path
            + "/"
            + args.agent_name.rsplit("_", 2)[0]
            + "/"
            + args.agent_name
        )
        ...
        if args.video:
            ...
            animation = GridAnimation(op_info=op_info, filename=args.filename, a=0.25)
            animation.render()
            end_time = time.time()
