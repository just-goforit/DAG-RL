...
class CustomMaskableActorCriticPolicy(MaskableActorCriticPolicy):
    ...
    def get_distribution(
        self, obs: th.Tensor, action_masks: Optional[np.ndarray] = None
    ) -> MaskableDistribution:
        """
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        self.action_net = (
            None
            if self.mlp_extractor.latent_dim_pi == 0
            else nn.Linear(self.mlp_extractor.latent_dim_pi, self.action_space.n)
        )
...
class CustomMlp(BaseFeaturesExtractor):
    ...
    * activation_fn
    ...
    def __init__(
        ...
        self.mlp = MlpLayer(
            in_features=(
                observation_space.shape[0] // self.node_num
                if self.node_wise
