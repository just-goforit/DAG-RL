...
class RedBluePebbleGameEnv(gym.Env):
    def __init__(self, config: Optional[dict] = None):
        ...
        self.compute_counts = np.zeros(self.state.node_num, dtype=np.uint16) # record node computed times
        self.fin_counts = 0
        self.mid_out = [] # mid nodes which have been output, decides game over or not
        self.valid_action_map = np.zeros((self.state.node_num, self.actual_node_action_size), dtype=np.bool_)
        self._reset_valid_action_map()
        ## Action space.
        self.action_space_aligned = config.get("align_action_space", False) # algined action space size for all types of node
        ...
        if self.obs_mode == 'vec':
            if self.obs_field == 'glb':
                self.observation_space = spaces.Box(low=0, high=65536, shape=(ADD_FEATURE + 
                                                                              self.g.input_num*IN_NODE_FEATURES +
                                                                              self.g.output_num*OUT_NODE_FEATURES +
                                                                              self.g.mid_num*MID_NODE_FEATURES,), dtype=np.float32)
            else:
                self.observation_space = spaces.Box(low=0, high=1.0, shape=(self.g.input_num*IN_NODE_FEATURES+
                                                                            self.g.output_num*OUT_NODE_FEATURES + 
                                                                            self.g.mid_num*MID_NODE_FEATURES,), dtype=np.float32)
        else:
            if self.obs_field == 'glb':
                self.observation_space = spaces.Box(low=0, high=1.0, shape=(ADD_FEATURE + 
                                                                            self.g.node_num*max(IN_NODE_FEATURES, 
                                                                                               MID_NODE_FEATURES, 
                                                                                               OUT_NODE_FEATURES),), dtype=np.float32)
            else:
                self.observation_space = spaces.Box(low=0, high=1.0, 
                                                    shape=(self.g.node_num*max(IN_NODE_FEATURES, 
                                                                               MID_NODE_FEATURES,
                                                                               OUT_NODE_FEATURES),), dtype=np.float32)
        ## constraint action
    ...
    def action_masks(self):
        ...
        """
        mask = self.wraped_cached_action_mask if self.wraped_cached_action_mask is not None else self._get_valid_action_mask()
        return mask
    ...
    def _get_obs(self):
        ...
        if self.obs_mode == 'vec':
            obs = np.concatenate((self.state.x[:self.g.input_num, 0:IN_NODE_FEATURES].flatten(), 
                                  self.state.x[self.g.input_num:self.g.input_num+self.g.output_num, 0:OUT_NODE_FEATURES].flatten(), 
                                  self.state.x[self.g.input_num+self.g.output_num:, 0:MID_NODE_FEATURES].flatten()))
            if self.obs_field == 'glb':
    ...
    def _agent_action_decode(self, idx):
        ...
        if idx >= self.g.input_num:
            if idx >= self.g.input_num + self.g.output_num:
                # mid node
                act = AGENT_MID_NODE_ACTS[np.argmax(node_actions[AGENT_MID_NODE_ACTION_MASK])]
                if node_actions[act] == False:
                    assert self.S == 0 and node_actions[DELETE] == True and self.state.x[idx, NODE_MEMED] > 0
                    # has in mem trans to DELETE
                    act = DELETE
            else:
                act = AGENT_OUT_NODE_ACTS[np.argmax(node_actions[AGENT_OUT_NODE_ACTION_MASK])]
        else:
    ...
    def _check_del_lock(self, node_id):
        ...
        if self.del_lock[node_id] == True:
            ...
            raise ValueError("node_id:{} in del lock but be choosed, del lock should not stunk game".format(node_id))
    ...
    def _implicit_load(self, node_id):
        ...
        self.S -= 1
        self.state._change_node_color_by_action(node_id, LOAD) # update node raw features
    ...
    def _implicit_del(self, node_id):
        ...
        self.S += 1
        self.state._change_node_color_by_action(node_id, DELETE) # update node raw features
    ...
    def _auto_delete(self, node_id):
        ...
        if self._is_midnode(node_id):
            ...
            for predecessor in predecessors:
                ...
                if len(successors) == 1 or np.all(self.state.x[successors, NODE_COMPUTED] > 0):
    ...
    def _auto_store(self, node_id):
        ...
        self.valid_action_map[node_id, DELETE] = self.valid_action_map[node_id, STORE] = self.valid_action_map[node_id, LOAD] = False
    ...
    def _get_valid_action_mask(self):
        ...
        if self.S == 0:
            ...
            tmp[:, LOAD] = False
            tmp[np.where(self.state.x[:, NODE_CACHED] < 1)[0], COMPUTE] = False # when S == 0 may still compute on output(has in cache already)
        else:
    ...
    def _is_action_valid(self, action:Union[int, tuple]):
        ...
        if self.render_mode: # human mode
            ...
            if action[0] >= self.state.node_num or action[1] >= self.actual_node_action_size:
                return False
    ...
    def _get_predecessors(self):
        ...
        for i in range(self.g.node_num - self.g.input_num):
            start_index = np.searchsorted(self._re_edges[:, 0], i + self.g.input_num, side='left')
            end_index = np.searchsorted(self._re_edges[:, 0], i + self.g.input_num, side='right')
    ...
    def _check_output_computability(self, node_id):
        ...
        if node_id < self.g.input_num:
            return False
        predecessors = self.get_predecessors(node_id)
        if np.any((self.state.x[predecessors, NODE_COMPUTED] > 0) & (self.state.x[predecessors, NODE_CACHED] > 0)): 
            return True
        return False
    ...
    def _is_output(self, node_id):
        return node_id >= self.g.input_num and node_id < self.g.input_num + self.g.output_num
        ...
        if self.verbose:
            if self.output_filename is None:
                ...
                with open(self.output_filename, "a+") as file:
                    ...
                    file.write('{} {} {}\n'.format(ACTS[act], node_id, '+' if prefix=='' else '*'))
    ...
    def _update_valid_action_space(self, node_id:int, act:int):
        ...
        #   4. compute-able[false]
            ...
            self.valid_action_map[node_id, STORE]  = True if node_is_output else False # lemma 1, but env2.0: for output, it can be stored
            self.valid_action_map[node_id, COMPUTE] = False if not node_is_output else self._check_output_computability(node_id) # lemma 4, but env2.0: for output, it may be computed-able
        # for it's successors:
        #   1. delete-able [still]
        #   2. load-able   [still]
        #   3. store-able  [still]
        ...
        #   4. compute-able[false]
            if not node_is_output:
                ...
                for successor in np.nditer(successors):
                    self.valid_action_map[successor, COMPUTE] = self._check_output_computability(successor)
        elif act == COMPUTE:
        ...
        #   3. store-able  [true]
        #   4. compute-able[false]
            self.valid_action_map[node_id, DELETE] = True if not node_is_output else False # ver 2.0
            self.valid_action_map[node_id, LOAD]  = False
            self.valid_action_map[node_id, STORE] = (self.state.x[node_id, NODE_MEMED] < 1) if not node_is_output else True
            self.valid_action_map[node_id, COMPUTE] = False # ver 2.0 we do compute autoly for all accessible mid dot node
        # for it's successors:
        #   1. delete-able [still]
        #   2. load-able   [still]
        #   3. store-able  [still]
        #   4. compute-able[check] *
            ...
            if self.del_lock_enable:
                if np.any(self.state.x[self.g.input_num:self.g.input_num+self.g.output_num, NODE_COMPUTED] < 1): 
        ...
        #   4. compute-able[check]
            ...
            self.valid_action_map[node_id, STORE]  = False
            self.valid_action_map[node_id, COMPUTE] = False # for input node never be computed, for mid node, delete is autoly after sucessor(output) computed, before that, it's could only be stored
        # for it's successors:
        #   1. delete-able [still]
        #   2. load-able   [still]
        #   3. store-able  [still]
    ...
    def step(self, action:Union[int, tuple]):
        ...
        if self._is_output(node_id) and act == COMPUTE:
            ...
            if self.state.x[node_id, NODE_CACHED] < 1 and self.state.x[node_id, NODE_MEMED] > 0:
        ...
        if act == DELETE:
            ...
            if self._is_midnode(node_id):
                ...
                # assert self._is_output(node_id)
                predecessors = self.get_predecessors(node_id)
                nums = np.count_nonzero((self.state.x[predecessors, NODE_COMPUTED] > 0) & (self.state.x[predecessors, NODE_CACHED] > 0) & (self.state.x[predecessors, NODE_CONTRIBUTED] < 1))
                assert nums > 0
                # self.state.x[node_id, NODE_COMPUTED] -= nums
                self.compute_counts[node_id] += nums
                self.state.x[node_id, NODE_COMPUTED] = float(self.compute_counts[node_id]) / float(self.g.output_indeg)
                reward += self.reward_config["COMPUTE"] * nums
                
        reward += self.reward_config["TIME_COST"]
        
        ## 3. put/remove pebble
        self.state._change_node_color_by_action(node_id, act) # update node raw features
        ...
        if act == COMPUTE and self._is_output(node_id) and self.compute_counts[node_id] < self.g.output_indeg:
            in_mem_mask = ((self.state.x[predecessors, NODE_COMPUTED] > 0) & (self.state.x[predecessors, NODE_MEMED] > 0) & (self.state.x[predecessors, NODE_CONTRIBUTED] < 1))
        ...
        if act == COMPUTE:
            if self.compute_counts[node_id] == self.g.output_indeg:
                ...
                if self.fin_counts == self.g.output_num:
                    ...
                    if self.summary:
                        print('Done reward sum:{:.3f} in {} steps'.format(self.rewd_sum + reward, self.steps))
                    terminated = True
        ## 7. truncate check
        truncated = False
        if not terminated and self.max_episode_len > 0 and self.steps >= self.max_episode_len:
            if self.summary:
                print('Truncated reward sum:{:.3f} at {} steps'.format(self.rewd_sum + reward, self.steps))
            truncated = True
