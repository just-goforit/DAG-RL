...
class operator_info:
    ...
    def __str__(self) -> str:
        return (
            "batch_size: %d, in_height: %d, in_width: %d, inplanes: %d, outplanes: %d, kernel_size: %d, stride: %d"
            % (
                self.batch_size,
                self.in_height,
                self.in_width,
...
class DAG_Graph:
    ...
    def __init__(
        ...
        if copy is not None:
            ...
            assert (
                output_indeg > 0
                and output_indeg < 65536
                and "output_indeg should be in range(0, 65536)"
            )
            ...
            self.x = np.zeros(
                (
                    self.node_num,
                    max(IN_NODE_FEATURES, OUT_NODE_FEATURES, MID_NODE_FEATURES),
                ),
                dtype=np.float32,
            )
...
def get_edges_naive_plane2plane(
    nodes_start: int,
    kern_start: int,
    out_start: int,
    in_height: int,
    ...
    * new_nodes_start(int, required): start node index of the new plane
    ...
    sample_plane = [
        [i * in_width + j + nodes_start for j in range(0, in_width)]
        for i in range(0, in_height)
    ]
    kernel_plane = [
        [i * kernel_size + j + kern_start for j in range(0, kernel_size)]
        ...
        for j in range((in_width - kernel_size) // stride + 1):
            ...
            for k in range(kernel_size):
                for l in range(kernel_size):
                    ...
                    t_product_edges.append(
                        (sample_plane[i * stride + k][j * stride + l], new_nodes_start)
                    )
...
def naive_Conv2d_DAG(op_info: operator_info, format: str = "nvis"):
    ...
    * stride(int, required[default=1]): stride
    ...
    # 2) output
    out_area = ((in_height - kernel_size) // stride + 1) * (
        (in_width - kernel_size) // stride + 1
    )  # a plane size of output
    ...
    for b in range(batch_size):
        ...
        for o in range(outplanes):
            ...
            for p in range(inplanes):
                ...
                addnode, addedge, new_nodes_start = get_edges_naive_plane2plane(
                    nodes_start=b * samples_vol + p * samples_area,
